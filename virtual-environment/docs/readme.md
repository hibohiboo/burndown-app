...
```
cinst -y git vagrant
cinst -y virtualbox --version=6.0.16
```

※ vagrant 2.2.6現在、`cinst -y virtualbox`で入ってくるバージョンだとエラーとなる。
...
virtual-envirionment/firebase/docker/.env

* `.env.sample`をコピーして作成すること
  * トークンは、`virtual-envirionment/firebase/login-token-generate.sh`を動かして出力された値を入力する
